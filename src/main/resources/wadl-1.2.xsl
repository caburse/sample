...
<!-- Global variables -->
<xsl:variable name="g_resourcesBase" select="ns2:application/ns2:resources/@base"/>

<!-- Template for top-level doc element -->
<xsl:template match="ns2:application">
    <html>
    <head>
    ...
    <table>
        ...
        <xsl:for-each select="ns2:resources/ns2:resource">
    ...
    <!-- Grammars -->
    <xsl:if test="ns2:grammars/ns2:include">
        <h2>Grammars</h2>
        <p>
            <xsl:for-each select="ns2:grammars/ns2:include">
    ...
    <!-- Detail -->
    <h2>Resources</h2>
    <xsl:for-each select="ns2:resources">
        <xsl:call-template name="getDoc">
            <xsl:with-param name="base" select="$g_resourcesBase"/>
        </xsl:call-template>
        <br/>
...
<xsl:template name="processResourceSummary">
    ...
    <xsl:if test="ns2:method">
        ...
        <xsl:if test="ns2:method and not($lastResource)">
    ...
    <xsl:for-each select="ns2:resource">
    ...
    <xsl:if test="ns2:method">
        ...
        <div class="methods">
            ...
            <div class="method">
                ...
                <xsl:choose>
                    <xsl:when test="ns2:request">
                        <xsl:for-each select="ns2:request">
                        ...
                        </xsl:for-each> <!-- ns2:request -->
                    </xsl:when>
    
                    <xsl:when test="not(ns2:request) and (ancestor::ns2:*/ns2:param)">
                        <xsl:call-template name="getParamBlock">
                            <xsl:with-param name="style" select="'template'"/>
                        </xsl:call-template>
                ...
                <div style="margin-left: 2em">  <!-- left indent -->
                <xsl:choose>
                    <xsl:when test="ns2:response">
                        <xsl:for-each select="ns2:response">
                            <div class="h8">status: </div>
                            <xsl:choose>
                                <xsl:when test="./ns2:representation/@status">
                                    <xsl:value-of select="./ns2:representation/@status"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    200 - OK
                                </xsl:otherwise>
                            </xsl:choose>
                            <xsl:for-each select="ns2:doc">
                                <xsl:if test="@title">
                            ...
                            <xsl:if test="ns2:param or ns2:representation">
                                <div style="margin-left: 2em"> <!-- left indent -->
                                <xsl:if test="ns2:param">
                                    <div class="h7">headers</div>
                                    <table>
                                        <xsl:for-each select="ns2:param[@style='header']">
...
<xsl:template name="getParamBlock">
    ...
    <xsl:if test="ancestor-or-self::ns2:*/ns2:param[@style=$style]">
        <div class="h7"><xsl:value-of select="$style"/> params</div>
        <table>
            <xsl:for-each select="ancestor-or-self::ns2:*/ns2:param[@style=$style]">
                <xsl:call-template name="getParams"/>
            </xsl:for-each>
        </table>
        <p/>
    ...
    <tr>
        <td><strong><xsl:value-of select="@name"/></strong></td>
            <td>
                ...
                <xsl:if test="ns2:option">
                    <br/>options:
                    <xsl:for-each select="ns2:option">
                        <xsl:choose>
        ...
        <xsl:if test="ns2:doc">
            <td><xsl:value-of select="ns2:doc"/></td>
        </xsl:if>
    </tr>
</xsl:template>
...
<xsl:template name="getRepresentations">
    <xsl:if test="ns2:representation">
        ...
        <table>
            <xsl:for-each select="ns2:representation">
                <tr>
...
<xsl:template name="getRepresentationParamBlock">
    <xsl:param name="style"/>
    <xsl:if test="ns2:param[@style=$style]">
        <tr>
            <td style="padding: 0em, 0em, 0em, 2em">
                <div class="h7"><xsl:value-of select="$style"/> params</div>
                <table>
                    <xsl:for-each select="ns2:param[@style=$style]">
                        <xsl:call-template name="getParams"/>
                    </xsl:for-each>
                </table>
                <p/>
...
<xsl:template name="getTitle">
    <xsl:choose>
        <xsl:when test="ns2:doc/@title">
            <xsl:value-of select="ns2:doc/@title"/>
        </xsl:when>
        <xsl:otherwise>
            Web Application
        </xsl:otherwise>
